...
def get_pretty_size(path):
    size = pretty_size = os.stat(path).st_size
    unit = 'B'
    decimals = 3
    GB, MB, KB = 1073741824, 1048576, 1024
    if (size > GB):
        pretty_size = round(size/GB, decimals)
        unit = 'GB'
    elif (size > MB):
        pretty_size = round(size/MB, decimals)
        unit = 'MB'
    elif (size > KB):
        pretty_size = round(size/KB, decimals)
        unit = 'KB'
